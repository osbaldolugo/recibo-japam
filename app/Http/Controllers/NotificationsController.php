...
class NotificationsController extends Controller
{
    ...
    {
        $notification_owner = auth()->user();
        $template = 'kordy.emails.transfer';
        $data = [
            'ticket'             => serialize($ticket),
            'notification_owner' => serialize($notification_owner),
            'original_ticket'    => serialize($original_ticket),
    ...
    {
        $notification_owner = auth()->user();
        $template = 'kordy.emails.assigned';
        $data = [
            'ticket'             => serialize($ticket),
            'notification_owner' => serialize($notification_owner),
        ];
    ...
    {
        Category::find($id)->with("agents");
    }

    public function inTicket($id)
    {
        Ticket::find($id)->where("agent_id","=",Auth::user()->id);
    }

    public function inOT($id)
    {
        dd($id);
    }
}
